...
class Command(BaseCommand):
    ...
    def _sync_project_id(self, project_id, workspace, models):
        ...
        if self.commit:
            ...
            field_names.remove('current_status')
        ...
        if Task in models and not project_dict['archived'] or self.process_archived:
            ...
            task_list = list(tasks_to_delete.values_list('remote_id', flat=True))
            tasks_to_delete.delete()
            message = "Successfully deleted {} tasks no Longer present: {}"
            message = message.format(len(task_list), task_list)
            self.stdout.write(self.style.SUCCESS(message))
            logger.info(message)
